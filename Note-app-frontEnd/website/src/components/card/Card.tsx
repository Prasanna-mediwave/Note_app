...
import Dispaly from "../../assets/Dispaly";
    ...
    <>
      {!state.viewDetial ? (
        ...
        >
          <div
            className={` ${
              state.cardDetial.length === 0 ? "hidden" : "max_xs:hidden"
            }`}
          >
            <motion.button
          ...
          <section
            className={`card-container ${
              state.view
                ? "grid-cols-1"
                : state.cardDetial.length === 0
                ? "grid-cols-1"
                : "grid-cols-2"
            }`}
          >
            {state.cardDetial &&
            state.cardDetial.length &&
            state.cardDetial.sort(
              (a: any, b: any) => b.pinnedNote - a.pinnedNote
            ) ? (
              state.cardDetial.map((item: any, index: any) => (
                <div
                  key={`card-id-${index}`}
                  onClick={() => {
                    axios
        ...
        <div
          ...
          style={{
            border: `${state.singleCard.noteColour}`,
          }}
        >
          <div className="card-header">{state.singleCard.title}</div>
          <div className="card-date">
            {moment(state.singleCard.createdOn).format("ll")}
          </div>
